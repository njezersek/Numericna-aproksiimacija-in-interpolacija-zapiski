...
Za B-zlepke stopnje $k$ in stopnje gladkosti $\nu_i = k-1$ velja:
\[ \left[\sum_{i=1}^{n+k}  c_i B_{i,k}(x)\right]' = k \sum_{i=1}^{n+k-1} \frac{c_{i+1} - c_i}{t_{i+k+1} - t_{i+1}} B_{i,k-1} \]

\[ \int_a^b \sum_{i=1}^{n+k}  c_i B_{i,k}(x) dx = \frac{1}{1+k} \sum_{i=1}^{n+k}(t_{i+k+1} - t_i)c_i \]
